...
import { LaptopRecommendationCard, LaptopSignalBreakdownItem } from "@/lib/types";
...
function ScoreBar({ score, weight }: { score: number; weight: number }) {
  ...
  const opacity = 0.4 + weight * 3; // higher weight = more opaque
  return (
...
function SignalRow({ item }: { item: LaptopSignalBreakdownItem }) {
...
export default function LaptopCard({ card, index }: Props) {
  const [showWatchOut, setShowWatchOut] = useState(false);
  const { device, rank, final_score, signal_breakdown, recommended_sku, why_recommended, watch_out, data_staleness_warning, use_case_scores } = card;
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {device.cpu} · {device.display_size}&quot; · {device.weight_kg}kg
          </p>
        </div>
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        >
          <div>
            ...
            >
              {recommended_sku
                ? `$${recommended_sku.price_usd.toLocaleString()}`
            ...
            {recommended_sku && (
              ...
              >
                {recommended_sku.ram_gb}GB RAM · {recommended_sku.storage_gb >= 1000 ? `${recommended_sku.storage_gb / 1000}TB` : `${recommended_sku.storage_gb}GB`} SSD
              </span>
            )}
            {recommended_sku?.notes && (
              <p
                style={{
                  margin: "2px 0 0",
                  fontSize: 11,
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
            ...
            >
              {device.os === "mac" ? "macOS" : device.os === "windows" ? "Windows" : "Linux"}
            </span>
