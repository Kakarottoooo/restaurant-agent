...
export default function RecommendationCard({ card, index }: Props) {
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className={`font-medium text-sm ${priceColor[r.price] ?? "text-gray-600"}`}>
                {r.price}
              </span>
          ...
          {card.watch_out && (
            <div className="mt-2 p-2.5 bg-amber-50 rounded-xl">
              <p className="text-xs font-semibold text-amber-700 mb-0.5">⚠️ Watch out</p>
          ...
          {/* Price estimate */}
          <p className="text-xs text-gray-500 mt-2">
            <span className="font-medium">Est. total:</span> {card.estimated_total}
          </p>
